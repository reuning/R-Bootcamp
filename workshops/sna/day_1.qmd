...
Double check the simple example early on. Something is wrong with the labeling. 
weighted=NA, in adjacency matrix 
Add: library(igraph) to the first time using igraph. 
...
For example using this [ground squirrel data](data/ground_squirrel_smith_2017a.graphml)

```{r}
net <- read_graph("data/ground_squirrel_smith_2017a.graphml", 
    format="graphml")
net
```
